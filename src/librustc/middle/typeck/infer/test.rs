...
impl<'a, 'tcx> Env<'a, 'tcx> {
    ...
    pub fn assert_not_subtype(&self, a: Ty<'tcx>, b: Ty<'tcx>) {
        if self.is_subtype(a, b) {
    ...
    pub fn t_int(&self) -> Ty<'tcx> {
        ty::mk_int()
    }

    pub fn t_rptr_late_bound(&self, binder_id: ast::NodeId, id: uint) -> Ty<'tcx> {
    ...
    pub fn check_no_lub(&self, t1: Ty<'tcx>, t2: Ty<'tcx>) {
        match self.lub().tys(t1, t2) {
            Err(_) => {}
            Ok(t) => {
    ...
    pub fn check_no_glb(&self, t1: Ty<'tcx>, t2: Ty<'tcx>) {
        match self.glb().tys(t1, t2) {
            Err(_) => {}
            Ok(t) => {
